<script lang="ts">
  ...
  const helperModule = esm`
  export const composedNoise = (xx, yy) => {
    let sum = 0;
    let freq = frequency;
    let amplitude = 1.0;
...
import { funTexture } from "${helperModule}";
export default function (x, y) {
   console.log("we're here");
  return funTexture(x, y);
}
`;

  let dataUri = esm`${customFunctionText}`;
  ...
  const update = () => {
    // import user-defined gen function
    const dataUri =
      "data:text/javascript;charset=utf-8," +
      encodeURIComponent(customFunctionText);
    import(dataUri).then((namespaceObject) => {
      const customFun = namespaceObject.default;
...
<div class="h-full p-4 w-full flex-1 flex flex-col text-black">
  ...
  <FormGroup>
    <FormLabel name="seed">Custom generator</FormLabel>
    <AceEditor
